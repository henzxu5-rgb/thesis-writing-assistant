...
> 读取 `library/<name>/` 目录下的所有 chunk 文件（每个使用 `limit: 50` 读前50行，
> 覆盖来源注释行+标题行+前几段正文。若 chunk 超过 50 行，再读最后 20 行以了解结尾内容）。
> 对每个 chunk 生成：
...
>    子话题应能帮助判断"某个具体论点是否在这个 chunk 中"。
>    若 chunk 较长（超过 50 行）且未能完整读取，对仅基于尾部 20 行推断的子话题标注 `[部分]`，
>    表示该话题可能不完整或存在遗漏。
>
