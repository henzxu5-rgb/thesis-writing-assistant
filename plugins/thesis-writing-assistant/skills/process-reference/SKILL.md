...
**🚫 强制停下**：向用户请求目录页，不得自行从 MD 或 PDF 中猜测目录结构。

> 已完成噪音清洗，接下来需要确认章节结构。
> 请将这本书的**目录页截图**或**目录文字**粘贴给我（可以是 PDF 截图、手机拍照、或复制粘贴）。
> 我会严格按照你提供的目录来划分层级。
...
> 根据目录，本书结构为：
> - 13 个章（#）：Preface, Kant on Law and Justice, The Innate Right...
> - 每章下若干节（##）：共约 45 节
> 
> 是否正确？确认后我开始修复标题层级。
...
**若不存在**，基于阶段一.5 中用户提供的目录信息编写 `fix-headings.py`：

> **信息来源**：阶段一.5 中用户已提供目录页（截图或文字），并确认了层级解读。以此为唯一权威依据。
>
> **辅助核对**：
> 1. **提取标题清单**：用 Grep 搜索 source-cleaned.md 中所有 `^#` 行及行号
> 2. 将 MD 中的标题清单与用户提供的目录逐一对照，找出层级不匹配或缺失的条目
> 3. **确认脚注格式**：请用户提供正文中一页有脚注的截图，或从 MD 中抽样识别脚注编号风格（上标、方括号、圆括号、数字+空格等）
...
**验证**：运行脚本后确认：
- 用 Grep 统计 source-fixed.md 中 `^# `、`^## `、`^### ` 的数量
- 与用户提供的目录对比：`#` 数 = 目录中章数，`##` 数 ≈ 目录中节数（±2），`###` 数 ≈ 目录中子节数
- 若书有脚注，脚注标记数 > 0

**🚫 硬性门禁——不通过不得继续到 Step 3.5**：

向用户展示对比表：
> | 层级 | 目录中的数量 | source-fixed.md 中的数量 | 差异 |
> |------|------------|------------------------|------|
> | # 章  | 13         | 13                     | 0    |
> | ## 节 | 45         | 44                     | -1   |

同时读取 source-fixed.md **前 30 行**和**后 30 行**，确认开头不是目录页/出版信息残留，结尾不是 Index/Bibliography 附录残留。

**若全部通过**：告知用户验证结果，继续到 Step 3.5。
**若任一项未通过**：**立即停下**，向用户展示具体差异，请用户人工检查 source-fixed.md。**不得自行重试或自动修复**，等待用户确认后再继续。
